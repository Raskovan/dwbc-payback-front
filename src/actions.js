...
function saveCategory(cityId, catName, catPrice, url) {
  const data = {
...
function shouldSaveCategory(state, cityId, catName) {
  ...
  const match = categories.filter(
    category => category.category_name === catName,
  )
  if (match.length === 0) {
    return true
  } else {
...
export function addCategoryForCity(cityId, catName, catPrice, url) {
  console.log(url)
  return (dispatch, getState) => {
    if (shouldSaveCategory(getState(), cityId, catName)) {
      return dispatch(saveCategory(cityId, catName, catPrice, url))
    }
  }
}
...
function cityExists(state, loginObj) {
  if (loginObj.city) {
    const foundCity = state.cities.cityList.filter(
